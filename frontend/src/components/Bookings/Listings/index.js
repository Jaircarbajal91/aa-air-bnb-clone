...
function Listing({ booking }) {
  ...
  let { id } = booking
  const [startDate, setStartDate] = useState(format(new Date(booking.startDate), 'MMM dd, yyy'))
  const [endDate, setEndDate] = useState(format(new Date(booking.endDate), 'MMM dd, yyy'))
    ...
    <div onClick={handleClick} className="single-booking-container">
      ...
      <div className="booking-content-container">
        <div className="spot-name-container">
          <span>{name}</span>
          <span>Spot hosted by <span className="user-name">{firstName} {lastName}</span></span>
        </div>
